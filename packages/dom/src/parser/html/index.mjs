...
export const parseHtmlDirty = (html, options) => {
  ...
  html = `<${TAGNAMES.ROOT}>${html}</${TAGNAMES.ROOT}>`;
  ...
  for (let i = data.length; (i -= 1) >= 0; undefined) {
    ...
    if (current.tagName) {
      ...
      if (current.tagName === TAGNAMES.COMMENT) text = current;
      else if (current.tagName === TAGNAMES.CDATA) text = current;
      else {
    ...
    if (
      ...
      if (tagName !== current.tagName) {
        if (current.tagName === CUSTOM_CLOSE) {
          ...
          current.tagName !== TAGNAMES.CDATA &&
          current.tagName !== TAGNAMES.COMMENT
        ) {
          continue;
        }
      }
...
export const parseHtml = (html, options) => {
  ...
  walk(tree, (node, parent) => {
    ...
    if (node.tag && node.tag[node.tag.length - 1] !== ONLY_CLOSE) {
      const [attrs, ...content] = node.children[0].wholeText.split(ONLY_CLOSE);
