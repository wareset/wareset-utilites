...
exports.parseHtml = exports.walk = exports.parseHtmlDirty = void 0;

var _caching = require("./lib/caching.js");

var _index = require("../../node-classes/index.js");
...
const parseHtmlDirty = (html, options) => {
  options = { ...DEFAULT_OPTIONS,
    ...(options || {})
  };
  let CACHE;
  [html, CACHE] = (0, _caching.caching)(html, options);
  ...
  html = `<${_index.TAGNAMES.ROOT}>${html}</${_index.TAGNAMES.ROOT}>`; // console.log(html);
  ...
  html.replace(regexp, (__, tag, tagName, pos) => {
    ...
    if (tagName === _index.TAGNAMES.DOCTYPE) {
  ...
  for (let i = data.length; (i -= 1) >= 0; undefined) {
    ...
    if (current.tagName) {
      ...
      if (current.tagName === _index.TAGNAMES.COMMENT) text = current;else if (current.tagName === _index.TAGNAMES.CDATA) text = current;else {
        if (!current.children) current.children = [];
        if (!text) current.children.shift();
        text = new _index.NODE_CLASSES[_index.TAGNAMES.TEXT](), text.__parent__ = current;
        text.start = pos + tag.length, text.end = lastPos;
        text.tagName = _index.TAGNAMES.TEXT;
        if (text.start !== text.end) current.children.unshift(text);
      }
      text.wholeText = wholeText; // (text.tag = '<#' + text.tagName + '>');

      if (current.tagName in _caching.CACHED_TAGS && wholeText in CACHE) {
    ...
    if (tag[1] === '/' || tag === SAMELESS_CLOSE || tag === COMMENT_CLOSE || tag === CDATA_CLOSE) {
      ...
      if (tag === COMMENT_CLOSE && options.comments) {
        tagName = _index.TAGNAMES.COMMENT, isComment = true;
      } else if (tag === CDATA_CLOSE && options.cdata) {
        tagName = _index.TAGNAMES.CDATA, isCDATA = true;
      }

      arr = new (_index.NODE_CLASSES[tagName] || _index.NODE_CLASSES[_index.TAGNAMES.ELEMENT])();
      ...
      if (tagName !== current.tagName) {
        ...
        } else if (current.tagName !== _index.TAGNAMES.CDATA && current.tagName !== _index.TAGNAMES.COMMENT) {
          continue;
        }
      }
      ...
      if (current.tagName === _index.TAGNAMES.CDATA || current.tagName === _index.TAGNAMES.COMMENT) {
...
exports.parseHtmlDirty = parseHtmlDirty;
...
const walk = (ast, cb) => __walk__(ast, cb);
