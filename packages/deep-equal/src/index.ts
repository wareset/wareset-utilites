...
const getCtor = (a: any): any => typed(a);
...
const getProtoOwnPropNames = (
  ...
  (typed.of(a) as Function[]).forEach((p) => {
    if (p && (natives || !isFuncNative(p))) {
      keys.push(...getOwnPropNames(p.prototype));
    }
  });
};
...
function __deepEqual__(
  ...
  if (a === b) return true;
  if (!depth || !isObj(a) || !isObj(b) || depth < 0) return a !== a && b !== b;

  const proto = getCtor(a);
  if (proto !== getCtor(b)) return false;

  if (!__cache__.has(a)) __cache__.set(a, b);
  else if (__cache__.get(a) === b) return true;

  if (isNum(depth)) (depth as number)--;
  ...
  let keys = getOwnPropNames(a);
  if (keys.length !== getOwnPropNames(b).length) return false;
  if (options.immerse && (options.natives || !isFuncNative(proto))) {
    getProtoOwnPropNames(a, keys, options.natives);
  }
  if (a instanceof Error) keys = keys.filter((v) => v !== 'stack');
  for (v of keys) {
  ...
  try {
    if (a.valueOf !== OBJ_PROTOTYPE.valueOf && isFunc(a.valueOf)) {
      // prettier-ignore
      return __deepEqual__(
        a.valueOf(), b.valueOf(), depth, options, __cache__);
    }

    if (a.toString !== OBJ_PROTOTYPE.toString && isFunc(a.toString)) {
      // prettier-ignore
      return __deepEqual__(
...
export function deepEqualExtended(
  a: any,
  b: any,
  options: Options = OPTIONS
): boolean {
